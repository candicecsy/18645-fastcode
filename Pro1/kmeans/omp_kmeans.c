...
{
    int i;
    float ans=0.0;   //answer
    float res[4];
    float res1[4];
    
    __m128 X, Y, temp;    //vectors of length 4
    __m128 X1, Y1, temp1;
    
    __m128 acc = _mm_setzero_ps(); //set to (0,0,0,0)
    __m128 acc1 = _mm_setzero_ps();

    //calculate the distance by 4 dimension once
    //loop unrolling
    for (i=0; i<=numdims - 8; i += 8)
    {
        X = _mm_loadu_ps(&coord1[i]);  //load the 4 coordinate
        Y = _mm_loadu_ps(&coord2[i]);
        
        temp = _mm_sub_ps(X, Y);       // y = (x1-x2)
        acc = _mm_add_ps(acc, _mm_mul_ps(temp,temp)); // y = y1^2+y2^2+...
        
        X1 = _mm_loadu_ps(&coord1[i+4]);  //load the 4 coordinate
        Y1 = _mm_loadu_ps(&coord2[i+4]);
        
        temp1 = _mm_sub_ps(X1, Y1);       // y = (x1-x2)
        acc1 = _mm_add_ps(acc1, _mm_mul_ps(temp1,temp1)); // y = y1^2+y2^2+...

    }
    _mm_storeu_ps(&res[0], acc);       //store the result to the res[4];
    _mm_storeu_ps(&res1[0], acc1);
    ans=res[0]+res[1]+res[2]+res[3]+res1[0]+res1[1]+res1[2]+res1[3];  // add the result
    
    //add the remaining values
    float substract;
    for(; i<numdims; i++)
    {
        substract = (coord1[i]-coord2[i]);
        ans +=  substract*substract;
    }
                   ...
                   int    *membership)        /* out: [numObjs] */
{
    ...
    is_perform_atomic = 0; // set the is_perform_atomic to zero
    ...
    do {
        ...
        //use the multiply to substitute the divide
        float newClusters_inv;
        for (i=0; i<numClusters; i++) {
            newClusters_inv = 1.0 / newClusterSize[i];
            for (j=0; j<numCoords; j++) {
                if (newClusterSize[i] > 1)
                    clusters[i][j] = newClusters[i][j] * newClusters_inv;
